...
//#region  Oversettelsestabeller
var translatingtable = [
  ["904","924","923"],
  ["709","707","708","725","726","727","712"],
  ["704","705","721"],
  ["106","103","113","131","133","134"],
  ["105","102","130","114","115"],
  ["1160","1154","1159","1214"],
  ["1231","1230"],
  ["612","601"],
...
//tar imot all dataen for 1 år, og fordeler det i egne arrays
...
// Henter årstall fra slider og sender http request som en get /api + params
// her kalles reqListner
...
function load()
{
  ...
  for(i in rootedIds)
  {
  	//var location; TODO ubrukt variabel
    ...
    if(index===-1)
    {
      ...
      for(t in translatingtable)
      {
        if(rootedIds[i]==translatingtable[t][0])
        {
          var v;
          var average=0;
          var correctAverage=0;
          var totalAreal=0;
          var totalJordAreal=0;
          var modifier=1;
          for(v in translatingtable[t])
          {
            if(v>0) //kan bli satt til true.. vil alltid være true
            { 
              var arrayindex = ids.indexOf(translatingtable[t][v])
              if(percentages[arrayindex])
              {
                totalAreal = totalAreal + Number(kareal[arrayindex]);
                totalJordAreal += (Number(kareal[arrayindex]) * Number(percentages[arrayindex]));

                //average=average+Number(percentages[arrayindex]);
              }
              else {
                modifier++;
              }
         ...
         // average=average/(translatingtable[t].length-modifier);
          correctAverage = totalJordAreal/totalAreal;
          //console.log('riktig ' + correctAverage + ' Dette er feil ' + average);
          colorize(correctAverage,rootedIds[i]);
          break
        }
      }
    }
...
//kalles fra index.html når søk knapp trykkes
function updateForm()
{
    ...
    for(i in rootedIds)
    {
      ...
      if(document.getElementById(id))
      {
        //setter inn navnet til området i søkefeltet og zoomer
